...
import matplotlib.pyplot as plt#for data visualtion
import seaborn#advanced data visulation
#loading the data
...
airlines.isna().sum()
airlines.isnull().sum()
#null or na values not found so imputation is not done
...
#ploting boxplot to check otliers presence
...
#scatter plot for bivariate data visualzation
...
#removing outliers and replacing with limits
...
#scaling data with minimax method
#user defining a function
def norm_fun(i):
...
from scipy.cluster.hierarchy import linkage#for linkage of hierarchical clustering
import scipy.cluster.hierarchy as sch#for hierarchical clustering

#linkage and ploting the dendrogram
...
from sklearn.cluster import AgglomerativeClustering#clustering using AgglomerativeClustering

#finding hierarchical clustering using AgglomerativeClustering method
...
#wring final file into some csv file
...
#checking duplicates of data and removing
dups1 = crime_data.duplicated()
sum(dups1)
crime_data = crime_data.drop_duplicates()

#ploting boxplot to check outliers presence
...
#scatterplot for bivariate data visulization
...
#defing a scaling function minimax scaling technique
def norm_func(i):
    x = (i-i.min())	/ (i.max()-i.min())
    return (x)
...
#importing oneHot encoder package
...
#boxploting and checking for outliers univatiate EDA
...
#defing scaling function standardization
def std_fun(i):
    x = (i-i.mean()) / (i.std())
    return (x)
...
#hierarchical clustering using diffrent linkages
...
#droping columns which have minimal insights
...
#checking for dupicates and removing
duplis = new_auto_data.duplicated()
sum(duplis)
new_auto_data = new_auto_data.drop_duplicates()

#boxploting and checking for outliers univariate EDA
...
#clsutering using AgglomerativeClustering
